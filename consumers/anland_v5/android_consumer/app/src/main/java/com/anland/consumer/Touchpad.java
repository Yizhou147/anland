...
public final class Touchpad {
    ...
    // self-use 个性化：禁用双指捏合/张开与三指及以上的触摸转发手势
    //（缩放等）。开启后这些手势被吞掉（不转发为触摸），双指滚动与
    // 双指右键点击不受影响。
    ...
    /** self-use 个性化：是否禁用双指捏合/张开与三指及以上的触摸转发手势。 */
    ...
    private boolean recognize(MotionEvent event) {
        ...
        // self-use 个性化：三指及以上手势被禁用时，吞掉整个流，直到手指
        // 减到一根（此时落回 ACTION_POINTER_UP 的正常处理恢复单指控制）。
